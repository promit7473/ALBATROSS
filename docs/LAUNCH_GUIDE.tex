...
\title{\vspace{-1.5em}ALBATROSS --- Terminal Launch Guide\\
...
\textbf{One-shot (recommended)} --- from the project root (e.g.\ \texttt{\textasciitilde/albatross}):
\begin{lstlisting}
bash launch/start_all.sh
\end{lstlisting}
This sources the ROS 2 / Gazebo environment, kills any stale instances, then starts
the web server and \texttt{launch/launch\_chattogram\_clean.sh} (SITL + chase cam +
bridge), all detached so they survive the terminal closing. Paths are self-locating,
so the folder can live anywhere and be named anything (override with
\texttt{MARITIME\_ROOT}). PX4-Autopilot and mavlink2rest are expected as symlinks in
\texttt{external/}.
...
source external/PX4-Autopilot/Tools/simulation/gz/setup.sh

# 2) GCS web server (:8080)
python3 gcs/server.py 8080

# 3) SITL + chase cam + bridge + mavlink2rest (in another terminal)
bash launch/launch_chattogram_clean.sh
\end{lstlisting}
...
\section{Arming \& mission flow}
The launcher \emph{auto-applies} the SITL-only arming bypass plus the smooth-flight
and yaw params about 12\,s after boot, so no manual step is normally needed. To set
them by hand, write to the pxh FIFO:
\begin{lstlisting}
...
\end{lstlisting}
Full flow in the UI: \textbf{Add waypoints} (each with its own height) $\rightarrow$
\textbf{Plan route} (routes around no-fly zones) $\rightarrow$ \textbf{Upload}
$\rightarrow$ \textbf{Arm} $\rightarrow$ \textbf{Start}.
