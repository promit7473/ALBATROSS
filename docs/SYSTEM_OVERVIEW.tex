...
\title{\vspace{-1.5em}Maritime UAV --- System Architecture \& Review Response\\
...
\subsection{Current directory layout (to be consolidated)}
\begin{lstlisting}
...
~/PX4-Autopilot   PX4 SITL + Chattogram gz world + models
~/mavlink2rest    MAVLink<->REST binary
~/launch_chattogram_clean.sh          one-shot SITL launcher
~/Chattogram ... Exclusion Zones.kml  operating area + no-fly zones
\end{lstlisting}
These live under \verb|$HOME| with absolute paths hard-coded in \texttt{server.py} and
the launcher; consolidating them into one project root is planned (see note at end).
